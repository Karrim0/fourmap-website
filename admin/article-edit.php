...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // لو نفس القيمة التلقائية أو فاضي → نحفظ NULL
    ...
    if ($title === '' || $content === '') {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadDir . $fileName)) {
                    if (!empty($article['image'])) {
                        $old = __DIR__ . '/../' . ltrim($article['image'], '/');
                        if (file_exists($old) && strpos($old, 'uploads/articles') !== false) unlink($old);
                    }
        ...
        if ($error === '') {
            $pdo->prepare("
                UPDATE articles
                SET title=?, excerpt=?, content=?, image=?, status=?, meta_title=?, meta_description=?
                WHERE id=?
            ")->execute([$title, $excerpt, $content, $imagePath, $status, $meta_title, $meta_description, $id]);
...
// القيم الحالية — POST values لو فيه error، لو لأ من DB
$curTitle   = $_SERVER['REQUEST_METHOD'] === 'POST' ? ($_POST['title']            ?? '') : $article['title'];
$curExcerpt = $_SERVER['REQUEST_METHOD'] === 'POST' ? ($_POST['excerpt']          ?? '') : ($article['excerpt'] ?? '');
$curMetaT   = $_SERVER['REQUEST_METHOD'] === 'POST' ? ($_POST['meta_title']       ?? '') : ($article['meta_title'] ?? '');
$curMetaD   = $_SERVER['REQUEST_METHOD'] === 'POST' ? ($_POST['meta_description'] ?? '') : ($article['meta_description'] ?? '');

// هل فيه تخصيص يدوي محفوظ؟
$isTitleCustom = !empty($article['meta_title']);
$isDescCustom  = !empty($article['meta_description']);
?>
  ...
  <div class="main-content">
    <div class="top-bar">
      ...
      <div class="top-bar-actions">
        <a href="../article.php?id=<?php echo (int)$article['id']; ?>" target="_blank"
           class="btn btn-outline-secondary btn-sm">
          <i class="bi bi-box-arrow-up-left me-1"></i> معاينة
        </a>
            ...
            <div class="card mb-4">
              ...
              <div class="card-body">
                ...
                <div class="mb-3">
                  ...
                  <textarea name="excerpt" id="article-excerpt" class="form-control"
                            rows="3"><?php echo htmlspecialchars($curExcerpt); ?></textarea>
                </div>

                <div class="mb-3">
                  <label class="form-label">محتوى المقال <span class="text-danger">*</span></label>
                  <textarea name="content" class="form-control" rows="16"><?php echo htmlspecialchars($article['content']); ?></textarea>
          ...
          <div class="col-lg-4">
              ...
              <div class="card-body">
                <div class="mb-3">
                  ...
                  <select name="status" class="form-select">
                    <option value="active"   <?php echo $article['status']==='active'   ? 'selected':''; ?>>منشور</option>
                    <option value="inactive" <?php echo $article['status']==='inactive' ? 'selected':''; ?>>مسودة</option>
                  </select>
                </div>
                <div class="mb-3 text-muted small">
                  <i class="bi bi-calendar3 me-1"></i>
                  نُشر في: <?php echo date('Y/m/d', strtotime($article['created_at'])); ?>
                </div>
            ...
            <div class="card mb-4">
              ...
              <div class="card-body">
                <?php if (!empty($article['image'])): ?>
                  <img src="../<?php echo htmlspecialchars(ltrim($article['image'],'/')); ?>"
                       class="img-fluid rounded mb-3"
                       style="max-height:180px; object-fit:cover; width:100%;"
                       onerror="this.style.display='none'">
                <?php endif; ?>
                <label class="form-label small text-muted">تغيير الصورة (اختياري)</label>
                <input type="file" name="image" class="form-control mb-2"
                       accept="image/*" onchange="previewImg(event,'imgPreview')">
                <div class="form-text mb-2">JPG / PNG / WEBP — الحد الأقصى 5MB</div>
            ...
            <div class="card seo-preview-card">
              <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <span class="seo-badge <?php echo ($isTitleCustom || $isDescCustom) ? 'seo-badge-custom' : 'seo-badge-auto'; ?>"
                      id="seo-badge">
                  <?php if ($isTitleCustom || $isDescCustom): ?>
                    <i class="bi bi-pencil"></i> مخصص
                  <?php else: ?>
              ...
              <div class="card-body">
                ...
                <div class="mb-3">
                  <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <button type="button" class="seo-reset-btn" id="reset-title"
                            style="display:<?php echo $isTitleCustom ? 'inline' : 'none'; ?>;"
                            onclick="resetTitle()">↺ إعادة تلقائي</button>
                  </div>
                  <input type="text" name="meta_title" id="meta-title"
                         class="form-control form-control-sm <?php echo $isTitleCustom ? 'seo-field-edited' : ''; ?>"
                         maxlength="70"
                         value="<?php echo htmlspecialchars($curMetaT ?: ($curTitle . ' | فور ماب')); ?>"
                ...
                <div class="mb-3">
                  <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <button type="button" class="seo-reset-btn" id="reset-desc"
                            style="display:<?php echo $isDescCustom ? 'inline' : 'none'; ?>;"
                            onclick="resetDesc()">↺ إعادة تلقائي</button>
                  </div>
                  <textarea name="meta_description" id="meta-desc"
                            class="form-control form-control-sm <?php echo $isDescCustom ? 'seo-field-edited' : ''; ?>"
                            rows="3" maxlength="160"
                            placeholder="سيُملأ تلقائياً من المقتطف..."><?php echo htmlspecialchars($curMetaD ?: mb_substr($curExcerpt, 0, 160)); ?></textarea>
                  <div class="seo-char-wrap">
                    <div class="seo-char-track">
                ...
                <div>
                  ...
                  <div class="google-preview-box">
                    <div class="gp-url">aaadosry.info/fourmap4/article.php?id=<?php echo (int)$article['id']; ?></div>
                    <div class="gp-title" id="gp-title-live">
                      <?php echo htmlspecialchars($curMetaT ?: ($curTitle . ' | فور ماب')); ?>
                    </div>
                    <div class="gp-desc" id="gp-desc-live">
                      <?php echo htmlspecialchars($curMetaD ?: mb_substr($curExcerpt, 0, 160)) ?: 'أضف مقتطفاً للمقال ليظهر هنا...'; ?>
                    </div>
                  </div>
                </div>
...
// القيم الحالية من DB
let autoTitle   = <?php echo json_encode($curTitle); ?>;
let autoDesc    = <?php echo json_encode(mb_substr($curExcerpt, 0, 160)); ?>;
let titleEdited = <?php echo $isTitleCustom ? 'true' : 'false'; ?>;
let descEdited  = <?php echo $isDescCustom  ? 'true' : 'false'; ?>;
...
document.getElementById('article-title').addEventListener('input', function () {
  ...
  if (!titleEdited) {
    ...
    updateBar('seo-title-bar', 'seo-title-count', (autoTitle + SUFFIX).length, 70, 50);
  }
  updatePreview();
});
